...
def split_data_by_year(
    df: pd.DataFrame, year_col: str
) -> Dict[str, pd.DataFrame]:
    """
    Split the dataframe into training, validation, 
    and test sets based on the year.
...
def create_inputs_targets(
    ...
    Returns:
        Dict[str, Any]: Dictionary containing inputs and targets for 
...
def impute_missing_values(data: Dict[str, Any], numeric_cols: list) -> None:
    ...
    Args:
        data (Dict[str, Any]): Dictionary containing inputs and 
        targets for train, val, and test sets.
...
def encode_categorical_features(
    ...
    Args:
        data (Dict[str, Any]): Dictionary containing inputs and targets for 
        train, val, and test sets.
        categorical_cols (list): List of categorical columns.
    """
    encoder = OneHotEncoder(sparse_output=False, handle_unknown='ignore').fit(
        data['train_inputs'][categorical_cols])
...
def preprocess_data(raw_df: pd.DataFrame) -> Dict[str, Any]:
    ...
    Returns:
        Dict[str, Any]: Dictionary containing processed inputs and 
        targets for train, val, and test sets.
    """
    ...
    numeric_cols = data['train_inputs'].select_dtypes(
        include=np.number).columns.tolist()[:-1]
    categorical_cols = data['train_inputs'].select_dtypes(
